...
void sha204_app_command_dispatch(void)
{
	switch (g_packet_block[0]) {	
		...
		if(status != ATCA_SUCCESS){
			printf("Unable to read Lock Status: ");
			sha204_parser_rc(status);
			return;
		}
		...
		if(read_data[3] == 0x55){
			...
			if(input == 'Y'){				
				...
				if(status != ATCA_SUCCESS){
					printf("Unable to Lock ConfigZone: ");
					sha204_parser_rc(status);
				}
				status = atcab_read_zone(ATCA_ZONE_CONFIG,0,2,5,read_data, 4);
				if(status != ATCA_SUCCESS){
		...
		case 0x07:
				...
				if(input == 'Y'){
					...
					if(status != ATCA_SUCCESS){
						printf("Unable to Lock Data Zone: ");
						sha204_parser_rc(status);
						return;
					}
		...
		case 0xA1:
			...
			if (status != ATCA_SUCCESS)
			{
				printf("Unable to run gendig/checkmac example: ");
				sha204_parser_rc(status);
			}
		...
		break;
		case 0xA3:
			status= sha204_create_diverse_key(root, 0x07, read_data, 0x77);
			if (status != ATCA_SUCCESS)
			{
				printf("Unable to calculate key: ");
				sha204_parser_rc(status);
			...
			status= sha204_create_diverse_key(root, 0x00, read_data, 0x77);
			if (status != ATCA_SUCCESS)
			{
				printf("Unable to calculate key: ");
				sha204_parser_rc(status);
		...
		case 0xA7:
			...
			*/
			...
			case 0xAA:	
				...
				if (status != ATCA_SUCCESS)
				{
					printf("Unable to sha204_validate_keys: ");
					sha204_parser_rc(status);
					return;
				}
			break;
			case 0xAB:
				...
				// mac is required to run dervied jey on this slot
			...
			case 0xAF:
				...
				if (status != ATCA_SUCCESS)
				{
					...
					key_04_roll_count=0;
					printf("Key 4 was set back to its orginal key. Roll out is 0.\n\r");
				}
			...
			case 0xB4:
				...
				if(status != ATCA_SUCCESS){	sha204_parser_rc(status);return;}					
				printf("Success!\n\r");
			break;
